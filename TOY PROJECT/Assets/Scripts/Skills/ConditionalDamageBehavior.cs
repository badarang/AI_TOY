...
public class ConditionalDamageBehavior : SkillBehavior
{
    ...
    public override UniTask ExecuteAsync(UnitBase caster, Vector2Int targetPos, Skill skill)
    {
        // 블랙보드에서 벽 충돌 여부 확인
        bool hitWall =
            skill.blackboard.ContainsKey("pushedUnitHitWall")
            && (bool)skill.blackboard["pushedUnitHitWall"];

        if (hitWall == triggerOnWallHit)
        {
            UnitBase target = skill.blackboard.ContainsKey("targetUnit")
                ? (UnitBase)skill.blackboard["targetUnit"]
                : null;
            if (target != null)
            {
                ...
                DebugPrinter.LogColor(
                    LogType.Unit,
                    $"ConditionalDamageBehavior: {target.name}에게 {finalDamage}의 조건부 데미지를 입혔습니다. (벽 충돌: {hitWall})"
                );
            }
        }
